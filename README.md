...
> 💡 **심사위원/면접관님을 위한 안내 (For Interviewers)**
> 본 프로젝트는 단순한 API 호출 챗봇을 넘어, **실제 문서 기반의 지식 검색(RAG 트랙라인)**, **서버 사이드 이벤트(SSE)를 활용한 실시간 스트리밍**, **반응형 최적화 UI**, 그리고 **에러 헨들링**을 종합적으로 고려하여 설계된 풀스택(Full-stack) 애플리케이션입니다. 하단의 아키텍처 및 특징 섹션을 통해 프로젝트의 기술적 고민과 해결 과정을 확인해 주시면 감사하겠습니다.
...
> 💡 **Note for Interviewers and Reviewers**
> This project is designed as more than just a simple API-wrapper chatbot. It is a full-stack application built with **document-based knowledge retrieval (RAG)**, **real-time Server-Sent Events (SSE) streaming**, a **responsive and optimized UI**, and **robust error handling**. Please review the architecture and features section below to see the technical depth and problem-solving approaches applied throughout this project.
