...
- [ ] **보안 강화 (Multi-layer Defense)**: 
    - [x] **1차 방어**: 시스템 프롬프트 강화 및 입력 데이터 태그화(XML)를 통한 탈옥 방지
    - [ ] **2차 방어**: Intent Router(의도 분류기)를 도입하여 도메인 외 질문(OOD) 전처리 차단
    - [ ] **3차 방어**: 벡터 검색 유사도 임계치(Similarity Threshold) 설정을 통한 무관한 답변 생성 방지
- [ ] **다국어 모델 및 성능 최적화**: 한국어 답변의 논리적 일관성 향상을 위한 전문 다국어 임베딩/LLM 모델 벤치마크 및 적용
